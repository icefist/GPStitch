...
def _discard(paths: list[str]) -> None:
    """Delete what preparation created, best effort - it is all reproducible."""
    for path in paths:
        try:
            Path(path).unlink(missing_ok=True)
        except OSError as e:  # noqa: PERF203 - one bad path must not strand the rest
            logger.warning("Could not remove %s after a failed merge: %s", path, e)
    ...
    try:
        ...
        if config.shared_gpx_path:
            ...
            for index, path in enumerate(paths, start=1):
                ...
                report = percentage_reporter(label, duration, on_progress)
            ...
            # Registered before it is written, not after: what has to be cleaned
            # up on the way out is whatever reached the disk, not whatever the
            # step managed to finish.
        ...
        # A failed preparation returns nothing, so nothing downstream knows what
        # it left behind - and a cancelled join has already written most of a
        # joined file, which for a long ride is tens of gigabytes of the space
        # the render itself is about to need.
        _discard(temp_files)
        raise
